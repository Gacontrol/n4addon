      ...
      <div className="flex-1 overflow-y-auto p-4 pl-5 space-y-5">
        ...
        {node.type === 'oneshot' && (
          <div className="space-y-3">
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  min={0}
                  step={100}
                  value={config.oneshotDurationMs ?? 1000}
                  onChange={e => updateConfig('oneshotDurationMs', parseInt(e.target.value) || 0)}
                  className="flex-1 bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-blue-500 transition-colors"
                />
                <span className="text-xs text-slate-400 flex-shrink-0">ms</span>
              </div>
              <p className="text-xs text-slate-500 mt-1">{((config.oneshotDurationMs ?? 1000) / 1000).toFixed(1)} Sekunden</p>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
                Aktiver Wert
              ...
              <input
                type="text"
                value={config.oneshotActiveValue !== undefined ? String(config.oneshotActiveValue) : 'true'}
                onChange={e => {
                  const val = e.target.value;
                  if (val === 'true') {
                    updateConfig('oneshotActiveValue', true);
                  } else if (val === 'false') {
                    updateConfig('oneshotActiveValue', false);
                  } else if (val === 'null') {
                    updateConfig('oneshotActiveValue', null);
                  } else if (!isNaN(Number(val)) && val !== '') {
                    updateConfig('oneshotActiveValue', Number(val));
                  } else {
                    updateConfig('oneshotActiveValue', val);
                  }
                }}
                className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-blue-500 transition-colors font-mono"
              />
              <p className="text-xs text-slate-500 mt-1">Wert waehrend Impuls (Zahl, true/false, null, Text)</p>
            </div>
            <div>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={config.oneshotInactiveIsNull !== false}
                  onChange={e => updateConfig('oneshotInactiveIsNull', e.target.checked)}
            ...
            {config.oneshotInactiveIsNull === false && (
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
                  Inaktiver Wert
                </label>
                <input
                  type="text"
                  value={config.oneshotInactiveValue !== undefined ? String(config.oneshotInactiveValue) : 'false'}
                  onChange={e => {
                    const val = e.target.value;
                    if (val === 'true') {
                      updateConfig('oneshotInactiveValue', true);
                    } else if (val === 'false') {
                      updateConfig('oneshotInactiveValue', false);
                    } else if (!isNaN(Number(val)) && val !== '') {
                      updateConfig('oneshotInactiveValue', Number(val));
                    } else {
                      updateConfig('oneshotInactiveValue', val);
                    }
                  }}
                  className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-blue-500 transition-colors font-mono"
                />
                <p className="text-xs text-slate-500 mt-1">Wert wenn nicht aktiv (Zahl, true/false, Text)</p>
              </div>
            )}
          </div>
        )}
